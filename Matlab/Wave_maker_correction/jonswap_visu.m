...
hold on
[theta,AG] = dirspread_salatin(wd,30*pi/180,Ndir,true);
%AG=AG/sum(AG.*(theta(2)-theta(1)));
...
plot(w,S);
hold on
plot(w,trapz(theta,tes));

%% Plot 3D spectra 
tes=S.*wa_bry_d';
disp("Avec dir : "+trapz(trapz(tes)));
disp("Sans dir : "+trapz(S));
