%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% FIGURE X : Correction of wave-maker related spurious standing 
% waves in a 3D wave-resolving model
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
clear all
close all
...
for it=1:2
    ...
    for j=1:length(sta)
        ...
        if it==1
            ...
            freqs=SMout1_tmp.freqs;
            dirs=SMout1_tmp.dirs;
            S1=S1+SMout1_tmp.S;
        elseif it==2
...
figure('PaperPositionMode', 'auto','Position',[100 100 1500 400]);
...
dw=(w(2)-w(1));
S=(wa*sqrt(8))^2/16*S/sum(S*dw); %see Suanda 2016;

scatter(w,S,70,'.','k'); % Input
hold on
plot(freqs,trapz(dirs,S2'),'LineWidth',3, ...
    'Color',[111,160,135,0.7*255]/255); % default
plot(freqs,trapz(dirs,S1'),'LineWidth',3, ...
    'Color',[192,24,135,0.7*255]/255);  % corrected
grid("minor");
fsize=19;
legend('Input','Default','Corrected','Interpreter','latex','FontSize',fsize-4);
ylabel('$D(f)$ $(m^2/Hz)$','Interpreter','latex','FontSize',fsize);
...
DSpecObs1=trapz(freqs,S1);
DSpecObs2=trapz(freqs,S2);
scatter(wd_bry*180/pi,wa_bry_d,70,'.','k'); % Input
hold on
plot(dirs,DSpecObs2/max(DSpecObs2),'LineWidth',3, ...
    'Color',[111,160,135,0.7*255]/255); % default
plot(dirs,DSpecObs1/max(DSpecObs1),'LineWidth',3, ...
    'Color',[192,24,135,0.7*255]/255);  % corrected
grid("minor");
legend('Input','Default','Corrected','Interpreter','latex','FontSize',fsize-4);
ylabel('$D(\theta)$ $(m^2/deg)$','Interpreter','latex','FontSize',fsize);
xlabel('$\theta$ (deg)','Interpreter','latex','FontSize',fsize);
xlim([-40+wd*180/pi,40+wd*180/pi]);
ylim([0 1.1]);

print(gcf, '-dpdf', '-bestfit',  './Figures/test');
...
%% Plotting only directional spread
...
%% Plotting only frequency spectrum
...
dw=(w(2)-w(1));
S=(wa*sqrt(8))^2/16*S/sum(S*dw);
FSpecObs1=trapz(dirs,S1');
FSpecObs2=trapz(dirs,S2');
scatter(w,S,'.','k'); %see Suanda 2016;
hold on
plot(freqs,FSpecObs2,'LineWidth',3, ...
    'Color',[111,160,135,0.7*255]/255);
plot(freqs,FSpecObs1,'LineWidth',3, ...
    'Color',[192,24,135,0.7*255]/255);
grid("minor");
%title('Frequency spectrum at X='+string(xpos)+' and Y='+string(ypos))
legend('Input','Default','Corrected','Interpreter','latex','FontSize',15);
